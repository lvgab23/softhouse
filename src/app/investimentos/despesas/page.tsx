...
import { z } from 'zod'
import { Plus, Search, ChevronDown, Trash2, TrendingDown, Home, Tag } from 'lucide-react'
...
const CATEGORIAS = [
  ...
  { value: 'iptu', label: 'IPTU / Taxas' },
  { value: 'manutencao', label: 'Manutenção' },
  { value: 'reforma', label: 'Reforma / Obra' },
  { value: 'seguro', label: 'Seguro' },
  { value: 'financiamento', label: 'Parcela Financiamento' },
  { value: 'consorcio', label: 'Parcela Consórcio' },
  { value: 'administracao', label: 'Taxa de Administração' },
...
const CAT_COLOR: Record<string, string> = {
  condominio:     'bg-blue-50 text-blue-700 border-blue-200',
  iptu:           'bg-red-50 text-red-700 border-red-200',
  manutencao:     'bg-amber-50 text-amber-700 border-amber-200',
  reforma:        'bg-orange-50 text-orange-700 border-orange-200',
  seguro:         'bg-teal-50 text-teal-700 border-teal-200',
  financiamento:  'bg-indigo-50 text-indigo-700 border-indigo-200',
  consorcio:      'bg-purple-50 text-purple-700 border-purple-200',
  administracao:  'bg-cyan-50 text-cyan-700 border-cyan-200',
...
const schema = z.object({
  patrimonio_id: z.string().min(1, 'Bem obrigatório'),
...
export default function BensDespesasPage() {
  ...
  const [bemFilter, setBemFilter]   = useState('todos')
  ...
  const fetchData = useCallback(async () => {
    ...
    const [d, p] = await Promise.all([
      ...
      supabase.from('patrimonios').select('id, nome').order('nome'),
    ])
    if (d.error?.code === '42P01' || d.error?.message?.includes('column "patrimonio_id"')) {
      setTableReady(false)
      setPatri(p.data || [])
      setLoading(false)
      return
  ...
  const filtered = useMemo(() => {
    let items = despesas
    if (bemFilter !== 'todos') items = items.filter(d => d.patrimonio_id === bemFilter)
    if (catFilter !== 'todos') items = items.filter(d => d.categoria === catFilter)
    if (search.trim()) {
      const q = search.toLowerCase()
      items = items.filter(d =>
        (d.patrimonios?.nome || '').toLowerCase().includes(q) ||
    ...
    return items
  }, [despesas, catFilter, bemFilter, search, periodoFilter])
  ...
  const bensComDespesa = new Set(filtered.map(d => d.patrimonio_id)).size
  ...
  return (
    <AppLayout>
      <Topbar title="Despesas — Bens" subtitle="Custos operacionais de imóveis e bens">
        {tableReady && (
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <MetricCard
                ...
                icon={Tag}
              />
              <MetricCard label="Bens com Despesas" value={String(bensComDespesa)} subtitle="ativos com custos" icon={Home} />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {[
                { v: periodoFilter, s: setPeriodo, opts: PERIODS },
                {
                  v: bemFilter,
                  s: setBemFilter,
                  opts: [{ value: 'todos', label: 'Todos os bens' }, ...patrimonios.map(p => ({ value: p.id, label: p.nome }))],
                },
                {
                  v: catFilter,
                  s: setCatFilter,
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50/50">
                        ...
                        <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Bem / Imóvel</th>
                      ...
                      {filtered.map((d: any, i: number) => {
                        ...
                        return (
                          <tr key={d.id} className={`border-b border-gray-50 hover:bg-gray-50/50 ${i === filtered.length - 1 ? 'border-b-0' : ''}`}>
                            ...
                            <td className="px-4 py-3 font-medium text-gray-900">{d.patrimonios?.nome || '—'}</td>
      ...
      >
        <form className="space-y-4">
          <Select label="Bem / Imóvel *" error={errors.patrimonio_id?.message} {...register('patrimonio_id')}>
          ...
          <Input label="Descrição" placeholder="Ex: Condomínio — maio/2025" {...register('descricao')} />
        </form>
      </Modal>
